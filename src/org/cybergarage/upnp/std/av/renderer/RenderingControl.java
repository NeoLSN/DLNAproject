...
public class RenderingControl extends AbstractService //implements ActionListener, QueryListener
{
    ////////////////////////////////////////////////
    // Constants
    ////////////////////////////////////////////////
    ...
    public RenderingControl(MediaRenderer render) throws InvalidDescriptionException
    {
        super(render);
    ...
    public boolean actionControlReceived(Action action)
    {
        ...
        if (actionName.equals(GETVOLUME)) {
            ...
            setArgumentValueFromRelatedStateVariable(action, CURRENTVOLUME);
            isActionSuccess = true;
        }

        if (actionName.equals(GETMUTE)) {
            String id = action.getArgumentValue(INSTANCEID);
            String channel = action.getArgumentValue(CHANNEL);

            setArgumentValueFromRelatedStateVariable(action, CURRENTMUTE);
            isActionSuccess = true;
        }

        if (actionName.equals(GETLOUDNESS)) {
            String id = action.getArgumentValue(INSTANCEID);
            String channel = action.getArgumentValue(CHANNEL);

            setArgumentValueFromRelatedStateVariable(action, CURRENTLOUDNESS);
            isActionSuccess = true;
        }

        if (actionName.equals(SETVOLUME)) {
            String volume = action.getArgumentValue(DESIREDVOLUME);
            // TODO: need a listener
            updateStateVariable(action, VOLUME, volume);
            isActionSuccess = true;
        }

        if (actionName.equals(SETMUTE)) {
            String mute = action.getArgumentValue(DESIREDMUTE);
            // TODO: need a listener
            updateStateVariable(action, MUTE, mute);
            isActionSuccess = true;
        }
        /*Jason*/
    ...
    public void updateStateVariable(Action action, String varname, String varvalue) {
        ...
        action.getService().getStateVariable(LASTCHANGE).setValue(toXmlString(action.getService()));
    }

    private String toXmlString(Service service) {
        String s = "<Event>\n<InstanceID val=\"0\">\n";
        ...
        for (int n = 0; n < tableSize; n++) {
            ...
            s += "<" + var.getName() + " val=\"" + XML.escapeXMLChars(var.getValue()) + "\"/>\n";
        }
        s += "</InstanceID>\n</Event>";
        return s;
    }
