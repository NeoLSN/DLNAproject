...
import org.cybergarage.upnp.std.av.data.ConnectionInfo;
import org.cybergarage.upnp.std.av.data.ConnectionInfoList;

import android.util.Log;

/*Jason*/
public class ConnectionManager extends AbstractService//implements ActionListener, QueryListener
/*Jason*/
{
    ////////////////////////////////////////////////
    // Constants
    ...
    public ConnectionManager(MediaRenderer render) throws InvalidDescriptionException
    {
        super(render);
        maxConnectionID = 0;
    ...
    public boolean actionControlReceived(Action action)
    {
        ...
        String actionName = action.getName();Log.i("Jason", "MediaRenderer - ConnectionManager - "+actionName);
        ...
        if (actionName.equals(PREPAREFORCONNECTION) == true) {
            action.getArgument(CONNECTIONID).setValue(getNewConnectionId());
        ...
        if (actionName.equals(GETPROTOCOLINFO) == true) {
            setArgumentValueFromRelatedStateVariable(action, SOURCE);
            setArgumentValueFromRelatedStateVariable(action, SINK);
            isActionSuccess = true;
        }
    ...
    private boolean getCurrentConnectionIDs(Action action)
    {
        ...
        /*Jason*/
        setRelatedStateVariableFromArgument(action, CONNECTIONIDS);
        /*Jason*/
        return true;
    }
    ...
    private synchronized boolean getCurrentConnectionInfo(Action action)
    {
        ...
        if (info != null) {
            ...
            action.getArgument(DIRECTION).setValue(ConnectionInfo.INPUT);
    ...
    protected synchronized int getNewConnectionId() {
        ...
        for (int n = 0; n < size; n++) {
            ConnectionInfo info = conInfoList.getConnectionInfo(n);
            if (info.getID() > currentHighestID) {
                currentHighestID = info.getID();
            }
        }
    ...
    protected void initializeStateVariables() {
        ...
        setStateVariable(SINKPROTOCOLINFO, "http-get:*:audio/mp3:*,http-get:*:video/mpeg4:*,http-get:*:video/mpg:*,http-get:*:image/jpeg:*,http-get:*:application/x-jpg:*");
